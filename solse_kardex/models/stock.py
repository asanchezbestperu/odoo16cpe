...
class stockMoveline(models.Model):
	...
	def _compute_reg_previus(self):
		...
		if len(self) > 1:
			for reg in self:
				if reg.move_id.company_id.calculate_stock_balance and reg.product_id.product_tmpl_id.type=='product' and reg.type_move in ['in', 'out'] and reg.id in _DATOS_JSON:
			...
			for reg in self:
				if reg.move_id.company_id.calculate_stock_balance and reg.product_id.product_tmpl_id.type=='product' and reg.type_move in ['in', 'out']:
					try:
