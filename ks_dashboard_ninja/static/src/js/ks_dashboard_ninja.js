...
//    const WebClient = require('web.WebClient');
	...
	var KsDashboardNinja = AbstractAction.extend({
		...
		init: function (parent, action) {
			...
			this.controllerID = action.controllerID;
			...
			this.ks_dashboard_id = action.params.ks_dashboard_id;
...
//            self.ks_fetch_data();
		...
		events: {
			...
			'click .ks_dashboard_more_action': 'onKsDashboardMenuContainerShow2',
			...
			return this._super().then(function(){
				const collaborationBusListener = ({ detail: notifications}) => {
		            for (const { payload, type } of notifications) {
		                if (type === "ks_dashboard_ninja.notification" && self.ks_mode ==='active') {
		                    var item_to_update = _(payload.changes).filter((x)=>{return self.ks_dashboard_data.ks_dashboard_items_ids.indexOf(x)>=0});
							if(item_to_update){
				...
				self.call('bus_service', 'addEventListener', 'notification', collaborationBusListener);
			});
		},
...
//                result = self.normalize_dn_data(result);
		...
		onKsDashboardMenuContainerShow2: function(e) {
			$(e.currentTarget).addClass('ks_dashboard_item_menu_show');
			var item_id = e.currentTarget.dataset.item_id;
			if (this.ksUpdateDashboard[item_id]){
				clearInterval(this.ksUpdateDashboard[item_id]);
			...
			//Dynamic Bootstrap menu populate Image Report
			if ($(e.currentTarget).hasClass('ks_dashboard_more_action')) {
				var chart_id = e.currentTarget.dataset.itemId;
				var name = this.ks_dashboard_data.ks_item_data[chart_id].name;
				var base64_image = this.chart_container[chart_id].toBase64Image();
				$(e.currentTarget).find('.dropdown-menu').empty();
				$(e.currentTarget).find('.dropdown-menu').append($(QWeb.render('ksMoreChartOptions', {
					href: base64_image,
					download_fileName: name,
					chart_id: chart_id
				})))
...
//                        "ks_board_active_user_ids": [[6, false, [odoo.session_info.uid]]],
	   ...
	   ks_add_dashboard_item_on_empty: function(e){
		...
		ksSum: function(count_1, count_2, item_info, field, target_1, $kpi_preview, kpi_data) {
			var self = this;
			var count = parseInt(count_1 + count_2);
		...
		renderKpi: function(item, grid) {
			...
			var item_info = {
				...
				target_progress_deviation: parseInt(Math.round((count_1 / target_1) * 100)) ? field_utils.format.integer(Math.round((count_1 / target_1) * 100)) : "0",
			...
			if (!kpi_data[1]) {
				...
				switch (field.ks_data_comparison) {
					case "None":
						...
						var count = parseInt(String(KsGlobalFunction.ksNumFormatter(count_1, 1))) + "/" + parseInt(String(KsGlobalFunction.ksNumFormatter(count_2, 1)));
		...
		onKsDuplicateItemClick: function(e) {
			...
			this._rpc({
				...
				self.displayNotification({ message: _t('Selected item is duplicated to ' + dashboard_name + ' .') });
		        //this.exit();
		...
		onKsMoveItemClick: function(e) {
			...
			this._rpc({
				...
				self.displayNotification({ message: _t('Selected item is moved to ' + dashboard_name + ' .') });
		        //this.exit();
				$.when(self.ks_fetch_data()).then(function() {
					$.when(self.ks_fetch_items_data()).then(function(){
						self.ks_remove_update_interval();
...
//                  self.ksUpdateDashboardItem()
		...
		_ksShowApplyClearDateButton: function(ks_l_none=false) {
